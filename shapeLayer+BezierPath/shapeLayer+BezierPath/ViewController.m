...
- (void)setUpButtons{
    NSArray *selectors = @[@"drawSJXLine",@"drawDBX",@"drawYUAN",@"drawSXJX",@"drawKXJX",@"drawYJJX",@"drawDYJJX",@"drawYH",@"drawZXHX",@"drawTwoBezier",@"drawThreeBezier",@"drawDuoLaAMeng11"];
    NSArray *titles = @[@"三角形",@"多边形",@"圆-椭圆",@"实心矩形",@"空心矩形",@"圆角矩形",@"单圆角矩形",@"圆弧",@"混合",@"二次贝塞尔",@"三次贝塞尔",@"哆啦A梦"];
...
- (void)drawDBX{
    ...
    [bezierpath addLineToPoint:CGPointMake(250, 200)];
    [bezierpath addLineToPoint:CGPointMake(200, 250)];
...
- (void)drawYUAN{
    // 矩形内切圆  椭圆
    [_shapeLayer removeFromSuperlayer];
    UIBezierPath *bezierPath = [UIBezierPath bezierPathWithOvalInRect:CGRectMake(30, 300, 200, 200)];
...
- (void)drawSXJX{
    ...
    layer.frame = (CGRect){100,200,100,100};
...
- (void)drawKXJX{
    [_shapeLayer removeFromSuperlayer];
    UIBezierPath *path = [UIBezierPath bezierPathWithRect:CGRectMake(100, 200, 100, 100)];
...
- (void)drawYJJX{
    [_shapeLayer removeFromSuperlayer];
    UIBezierPath *path = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(100, 200, 200, 100) cornerRadius:50];
    _shapeLayer = [CAShapeLayer layer];
    _shapeLayer.path = path.CGPath;
...
- (void)drawDYJJX{
    [_shapeLayer removeFromSuperlayer];
    UIBezierPath *path = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(100, 200, 200, 100) byRoundingCorners:UIRectCornerTopLeft cornerRadii:(CGSize){100,0}];
    _shapeLayer = [CAShapeLayer layer];
    _shapeLayer.path = path.CGPath;
    _shapeLayer.fillColor = [UIColor redColor].CGColor;
    _shapeLayer.strokeColor = [UIColor clearColor].CGColor;
...
- (void)drawYH{
    [_shapeLayer removeFromSuperlayer];
    // center:圆弧中心(相对) radius:半径 starAngle:开始角度 endAngle:结束点角度 clockwise:是否顺时针
    UIBezierPath *path = [UIBezierPath bezierPathWithArcCenter:CGPointMake(100, 200) radius:50 startAngle:0 endAngle:M_PI_2 clockwise:NO];
    _shapeLayer = [CAShapeLayer layer];
    _shapeLayer.path = path.CGPath;
    _shapeLayer.fillColor = [UIColor clearColor].CGColor;
    _shapeLayer.strokeColor = [UIColor redColor].CGColor;
...
- (void)drawZXHX{
    ...
    [path addArcWithCenter:CGPointMake(200, 250) radius:50 startAngle:0 endAngle:M_PI clockwise:YES];
    _shapeLayer = [CAShapeLayer layer];
    _shapeLayer.path = path.CGPath;
...
- (void)drawTwoBezier{
    ...
    [path1 moveToPoint:CGPointMake(100, 200)];
    [path1 addQuadCurveToPoint:CGPointMake(300, 300) controlPoint:CGPointMake(200, 200)];
    _shapeLayer = [CAShapeLayer layer];
...
// 三次贝塞尔曲线
- (void)drawThreeBezier{
    [_shapeLayer removeFromSuperlayer];
    UIBezierPath *path = [UIBezierPath bezierPath];
    [path moveToPoint:CGPointMake(100, 200)];
    [path addCurveToPoint:CGPointMake(300, 300) controlPoint1:CGPointMake(200, 200) controlPoint2:CGPointMake(150, 250)];
    _shapeLayer = [CAShapeLayer layer];
    _shapeLayer.path = path.CGPath;
    _shapeLayer.fillColor = nil;
    _shapeLayer.strokeColor = [UIColor redColor].CGColor;
...
#define LanPangZiDuration 1
//- (void)drawDuoLaAMeng{
//    [_shapeLayer removeFromSuperlayer];
//    CGFloat delay = LanPangZiDuration;
//    // 头
//    CAShapeLayer *headLayer = [CAShapeLayer layer];
//    UIBezierPath *headPath = [UIBezierPath bezierPathWithOvalInRect:CGRectMake(100, 150, 120, 120)];
//    headLayer.path = headPath.CGPath;
//    headLayer.fillColor = Color(0, 147, 212).CGColor;
//    headLayer.strokeColor = [UIColor grayColor].CGColor;
//    [self.view.layer addSublayer:headLayer];
//    
//    // 脸
//    CAShapeLayer *faceLayer = [CAShapeLayer layer];
//    UIBezierPath *facePath = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(110, 190, 100, 50) byRoundingCorners:UIRectCornerAllCorners cornerRadii:CGSizeMake(120, 0)];
//    faceLayer.path = facePath.CGPath;
//    faceLayer.fillColor = [UIColor whiteColor].CGColor;
//    faceLayer.strokeColor = [UIColor grayColor].CGColor;
//    [self.view.layer addSublayer:faceLayer];
//    
//    // 眼睛
//    CAShapeLayer *leftEyeLayer = [CAShapeLayer layer];
//    UIBezierPath *leftEyePath = [UIBezierPath bezierPathWithRoundedRect:<#(CGRect)#> byRoundingCorners:<#(UIRectCorner)#> cornerRadii:<#(CGSize)#>];
//    leftEyeLayer.path = leftEyePath.CGPath;
//    leftEyeLayer.fillColor = [UIColor grayColor].CGColor;
//    leftEyeLayer.strokeColor = [UIColor whiteColor].CGColor;
//    [self.view.layer addSublayer:leftEyeLayer];
//}


- (void)drawDuoLaAMeng11
{
...
- (void)setLayer:(CAShapeLayer *)layer path:(UIBezierPath *)path delay:(CFTimeInterval)delay
{
    ...
    layer.fillColor = Color(0, 141, 212).CGColor;
    layer.strokeColor = [UIColor lightGrayColor].CGColor;
    
    __weak typeof(self) weakSelf = self;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [weakSelf.view.layer addSublayer:layer];
//        [weakSelf.view.layer addAnimation:layer duration:LanPangZiDuration];
    });
}
